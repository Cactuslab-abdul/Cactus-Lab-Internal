...
export default function PortalAdminPage() {
  ...
  // Try to find the slug for this client from the seed or from localStorage clients
  ...
  useEffect(() => {
    ...
    // Try to find in localStorage clients first
    try {
  ...
  useEffect(() => {
    ...
    fetch(`/api/portal/admin/${slug}`)
      .then(async r => {
        ...
        // If not found in Supabase, fall back to seed data
  ...
  const save = useCallback(async (updated: PortalData) => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        // If Supabase not available, just keep it in local state for this session
        setData(updated);
        setSaved(true);
        setTimeout(() => setSaved(false), 2500);
      }
  ...
  if (!data) {
    return (
      <div className="space-y-6 fade-in">
        <Link href="/clients" className="flex items-center gap-2 text-[#555] hover:text-white text-sm transition-colors">
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <Link href="/clients" className="flex items-center gap-2 text-[#555] hover:text-white text-sm transition-colors mb-3">
            <ArrowLeft className="w-4 h-4" />
            Back to Clients
          </Link>
      ...
      <div className="flex items-center gap-3 bg-[#111] border border-[#1e1e1e] rounded-xl px-4 py-3">
        ...
        <code className="text-green-400 text-sm font-mono flex-1 truncate">{typeof window !== "undefined" ? `${window.location.origin}${portalUrl}` : portalUrl}</code>
        <button
          onClick={() => navigator.clipboard.writeText(typeof window !== "undefined" ? `${window.location.origin}${portalUrl}` : portalUrl)}

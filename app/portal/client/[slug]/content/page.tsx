...
const STATUS_CFG: Record<VideoStatus, { label: string; color: string; dot: string; actionable: boolean }> = {
  idea_pending:     { label: "Awaiting Approval", color: "text-amber-400 bg-amber-500/10 border-amber-500/20",   dot: "bg-amber-400",  actionable: true },
  idea_revision:    { label: "Revision Requested", color: "text-red-400 bg-red-500/10 border-red-500/20",        dot: "bg-red-400",    actionable: false },
  idea_approved:    { label: "Idea Approved",      color: "text-green-400 bg-green-500/10 border-green-500/20",  dot: "bg-green-400",  actionable: false },
  in_production:    { label: "In Production",      color: "text-blue-400 bg-blue-500/10 border-blue-500/20",     dot: "bg-blue-400",   actionable: false },
  ready_for_review: { label: "Ready for Review",   color: "text-purple-400 bg-purple-500/10 border-purple-500/20", dot: "bg-purple-400", actionable: true },
  client_approved:  { label: "Approved ✓",         color: "text-green-400 bg-green-500/10 border-green-500/20",  dot: "bg-green-400",  actionable: false },
  posted:           { label: "Live",               color: "text-teal-400 bg-teal-500/10 border-teal-500/20",     dot: "bg-teal-400",   actionable: false },
};
...
function VideoModal({ video, onClose, onUpdated }: {
  ...
  const submitComment = async () => {
    ...
    if (res.ok) { const c = await res.json(); setComments(prev => [...prev, c]); setCommentText(""); setTimestamp(""); }
    setSending(false);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {/* Approve / Revise — prominent at top when actionable */}
          {cfg.actionable && !revisionOpen && (
...
export default function ContentPage() {
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      {/* Revision modal */}
      {reviseTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setReviseTarget(null)}>
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" />
